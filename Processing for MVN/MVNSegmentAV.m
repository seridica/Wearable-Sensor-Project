...
function [segmentNames, segmentData] = MVNSegmentAV(mvnStruct)
    ...
    for i=4:nFrames
        ...
        %segmentPosition = strsplit( mvnStruct.mvnx.subject.frames.frame{i}.velocity.Text, ' ');
        segmentOrientation = strsplit( mvnStruct.mvnx.subject.frames.frame{i}.angularVelocity.Text, ' ');
        ...
        for j=1:numSegments
            ...
            segmentData(i-3,(j-1)*6+4+1) = 0; %str2double( segmentPosition{(j-1)*3+1} );
            segmentData(i-3,(j-1)*6+5+1) = 0; %str2double( segmentPosition{(j-1)*3+2} );
            segmentData(i-3,(j-1)*6+6+1) = 0; %str2double( segmentPosition{(j-1)*3+3} );
        end
    end
end