%%%
% File name: MVNParse.m
% Author: Calvin Kuo
% Date: 9/20/2018
% 
% Parse the MVNX xml file. First, convert to a struct using the xml2struct
% function provided on matlab fileshare. Then, extract number of sensors
% and all sensor data into a n*(m+1) matrix (n time stamps, m sensors and a
% time column).

function [mvnStruct, sensorNames, sensorData] = MVNParse(filename)
    % Read and convert xml file
    xmltemp = xmlread(filename);
    mvnStruct = xml2struct(xmltemp);
